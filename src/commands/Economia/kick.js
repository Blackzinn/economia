...
exports.run = (client, message, args) => {
  ...
  .addField(`:book: **Exemplo**`, `\`${c.prefix}kick @young Totoso\``, true)
  ...
  .setColor('#a67dff')  
       ...
       if (membro === message.member) return message.channel.send(`<a:nop:698221501668655124> **»** Kon'nichiwa **${message.author.username}**, você não pode se banir!`)
       
       var motivo = args.slice(1).join(" ");
       if (!motivo) return message.channel.send(erro);
       
       if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send(`<:alert:696822589208920124> **»** Kon'nichiwa **${message.author.username}**, você não possui a permissão \`BAN_MEMBERS\`.`)
       ...
       message.channel.send(banembed).then(msg => { 
         ...
         coletor.on("collect", em => { 
             ...
             .setColor('#0000')
             var canal = message.guild.channels.get(db.get(`punichannel_${message.guild.id}`))
             if (!canal) {

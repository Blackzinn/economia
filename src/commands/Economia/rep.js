...
exports.run = async (client, message, args) => { 
  ...
  .addField(`:book: **Exemplo**`, `\`${c.prefix}rep @young\``, true)
  ...
  .setColor('#a67dff')    
    ...
    if (tempo !== null && timeout - (Date.now() - tempo) > 0) {
        ...
        message.channel.send(`<:alert:696822589208920124> **»** Kon'nichiwa **${message.author.username}**, você já deu uma reputação há pouco tempo! Tente novamente em: \`${time.hours}h ${time.minutes}m ${time.seconds}s\``)
} else {

...
exports.run = async (client, message, args) => {
  ...
  .setColor('#a67dff')  
      if (!message.guild.me.hasPermission('MANAGE_CHANNELS')) return message.channel.send(`<:alert:696822589208920124> **»** Kon'nichiwa **${message.author.username}**, eu necessito da permissão \`MANAGE_CHANNELS\` para criar um canal.`)
        if (!message.member.hasPermission('MANAGE_CHANNELS')) return message.channel.send(`<:alert:696822589208920124> **»** Kon'nichiwa **${message.author.username}**, você precisa da permissão \`MANAGE_CHANNELS\`.`)
      ...
      message.channel.send(`<a:yep:698221405434806282> » Canal criado com sucesso!`).then(() => {
          message.guild.createChannel(args[1], args[0], []).catch((err) => {
            message.channel.send(`<a:nop:698221501668655124> **»** Kon'nichiwa **${message.author.username}**, ocorreu um erro ao criar esse canal!`)
          });
        });
      } catch (err) {
